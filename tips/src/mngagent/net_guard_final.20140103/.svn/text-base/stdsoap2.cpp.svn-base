	...
	if (nwritten < 0)
	  ...
	  { soap->errnum = err;
        ...
        nwritten = send(sk, s, (int)n, soap->socket_flags);
				printf("fsend  __LINE__=%d nwritten=%d\n", __LINE__, nwritten);
#else
      ...
      if (nwritten <= 0)
      {
        ...
        if (err == SOAP_EWOULDBLOCK || err == SOAP_EAGAIN)
        {
          ...
          if (!r && soap->send_timeout)
					{
						printf("fsend  __LINE__=%d r=%d soap->send_timeout=%d\n", __LINE__,r, soap->send_timeout);
            return SOAP_EOF;
					}
...
{
	printf("soap_flush_raw  __LINE__=%d\n", __LINE__);
	if ((soap->mode & SOAP_IO) == SOAP_IO_STORE)
  { register char *t;
...
{
	printf("__LINE__=%d  soap_recv_raw start\n", __LINE__);
	register size_t ret;
	printf("__LINE__=%d  soap_recv_raw ret=%d\n", __LINE__, (ret));
  ...
  { if (soap->d_stream->next_out == Z_NULL)
	...
	printf("__LINE__=%d  soap_recv_raw\n", __LINE__);
      return EOF;
    }
...
zlib_again:
  ...
  { 
		printf("__LINE__=%d  soap_recv_raw if ((soap->mode & SOAP_IO) == SOAP_IO_CHUNK)  ret=%d\n", __LINE__, (ret));
		for (;;)
...
soap_recv(struct soap *soap)
{
	...
	printf("__LINE__=%d  soap_recv start\n", __LINE__);
#ifndef WITH_LEANER
...
{ register soap_wchar c;
  c = soap->ahead;
//	printf("__LINE__=%d  soap_getchar c=%d\n", __LINE__, c);
  if (c)
  { if (c != EOF)
      soap->ahead = 0;
//	printf("__LINE__=%d  soap_getchar c=%d\n", __LINE__, c);
...
//	printf("__LINE__=%d  soap_get1 start c=%d\n", __LINE__, c);
	c = soap_get1(soap);
//	printf("__LINE__=%d  soap_getchar c=%d\n", __LINE__, c);
  return c;
}
...
{
			int ret = 0;
		if (soap->bufidx >= soap->buflen && (ret = soap_recv(soap)))
				{
		printf("__LINE__=%d  soap_get1 return EOF\n", __LINE__);
    return EOF;
				}
	unsigned char c = (unsigned char)soap->buf[soap->bufidx++];
	printf("__LINE__=%d  soap_get1 return EOF\n", __LINE__);
	fflush(stdout);
  return c;
}
...
{
	register int r;
//  printf("tcp_select  __LINE__=%d r=%d\n", __LINE__,r);
  struct timeval tv;
...
{
#ifndef WITH_LEANER
	printf("soap_end_send __LINE__=%d\n", __LINE__);
  int err;
  if (soap->dime.list)
...
{
	printf("__LINE__=%d  soap_begin_recv start\n", __LINE__);
	register soap_wchar c;
  ...
  *soap->endpoint = '\0';
  ...
  if (soap->fprepareinitrecv && (soap->error = soap->fprepareinitrecv(soap)))
	{
		printf("__LINE__=%d  soap_begin_recv\n", __LINE__);
    return soap->error;
	}
#endif
	printf("__LINE__=%d  soap_begin_recv c=%d\n", __LINE__, c);
  c = soap_getchar(soap);
	printf("__LINE__=%d  soap_begin_recv c=%d\n", __LINE__, c);
#ifdef WITH_GZIP
  if (c == 0x1F)
  ...
  if ((int)c == EOF)
	{
		printf("__LINE__=%d  soap_begin_recv EOF=%d\n", __LINE__, EOF);
    return soap->error = SOAP_CHK_EOF;
	}
  soap_unget(soap, c);
#ifndef WITH_NOHTTP
...
#endif
	printf("__LINE__=%d  soap_begin_recv end\n", __LINE__);
  return SOAP_OK;
}
#endif
