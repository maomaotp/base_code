...
int client_init(struct soap *soap,int type)
{
	...
	sprintf(Server,"172.16.0.139:8088");
...
int get_device_state(struct soap *soap,struct ns__MsgList *state,struct status_struct *status)
{
	...
	memset(&(state->__ptr[0]),0,sizeof(struct ns__MsgItem));
	memset(temp,0,sizeof(temp));
	sprintf( temp,"<device_id>%s</device_id><stime>%s</stime><mem>%d</mem><cpu>%d</cpu><disk>%0.2lf</disk><mirror>%c</mirror><flow>%d</flow><active_peer>%d</active_peer><module_state>%s</module_state>",status->device_id,status->stime,status->mem,status->cpu,status->disk,status->mirror,status->flow,status->active_peer,status->module_state);
