...
int get_device_state(struct soap *soap,struct ns__MsgList *state,struct status_struct *status)
{
	...
	memset(temp,0,sizeof(temp));
	sprintf( temp,"<device_id>%s</device_id><stime>%s</stime><mem>%d</mem><cpu>%d</cpu><disk>%0.2lf</disk><mirror>%c</mirror><flow>%d</flow><active_peer>%d</active_peer><module_state>%s</module_state>",status->device_id,status->stime,status->mem,status->cpu,status->disk,'1',status->flow,status->active_peer,"11310000");
  cout <<"temp  =============================== " << temp<<endl;
  //exit(0);
