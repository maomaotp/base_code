...
char descry[65];
	...
	//if(1 != ((struct dns_rules *)rule)[i].type)
		...
		//			memset((state->__ptr+i),0,sizeof(struct ns__MsgItem));
					...
					sprintf( temp,"<alarm_id>%s</alarm_id><device_id>%s</device_id><trojan_id>%s</trojan_id><sip>%s</sip><dip>%s</dip><dmac>%s</dmac><alarm_time>%s</alarm_time><type>%s</type><risk>%c</risk><scc>%s</scc><scc_flag>%c</scc_flag><descr>%s</descr>",(*data + i)->alarm_id,(*data + i)->device_id,(*data + i)->trojan_id,(*data + i)->sip,(*data + i)->dip,(*data + i)->dmac,(*data + i)->alarm_time,(*data + i)->type,(*data + i)->risk,(*data + i)->scc,(*data + i)->scc_flag != '/'?(*data + i)->scc_flag: '0',(*data + i)->descry);
