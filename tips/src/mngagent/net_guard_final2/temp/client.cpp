...
//int SendData (struct alarm** alm, struct alert** alt, struct event** en, struct block** bk)//一次上传多条消息，成功返回0，失败返回1
...
int get_device_data(struct soap *soap,struct ns__MsgList *state,void **data,int size,int type)
{
	...
	{
		case 1://alarm
			data = (struct alarm**)data;
			for(i=0;i<size;i++)
			{
				memset(&(state->__ptr+i),0,sizeof(struct ns__MsgItem));
				memset(temp,0,sizeof(temp));
				sprintf( temp,"<alarm_id>%s</alarm_id><device_id>%s</device_id><trojan_id>%s</trojan_id><sip>%s</sip><dip>%s</dip><dmac>%s</dmac><alarm_time>%s</alarm_time><type>%s</type><risk>%s</risk><scc>%s</scc><scc_flag>%s</scc_flag><descr>%s</descr>",data[i]->alarm_id,data[i]->device_id,data[i]->trojan_id,data[i]->sip,data[i]->dip,data[i]->dmac,data[i]->alarm_time,data[i]->type,data[i]->risk,data[i]->scc,data[i]->scc_flag,data[i]->descr);
			...
			break;
		case 2://alert
			data = (struct alert**)data;
			for(i=0;i<size;i++)
			{
				memset(&(state->__ptr+i),0,sizeof(struct ns__MsgItem));
				memset(temp,0,sizeof(temp));
				sprintf( temp,"<alert_id>%s</alert_id><device_id>%s</device_id><plugin_id>%d</plugin_id><risk>%s</risk><trojan_id>%s</trojan_id><alarm_time>%s</alarm_time><sip>%s</sip><sport>%d</sport><dip>%s</dip><dport>%d</dport><dmac>%s</dmac><protocol>%hd</protocol><descr>%s</descr><dnshash>%s</dnshash><scc>%s</scc><scc_flag>%s</scc_flag><extra_text>%s</extra_text>",data[i]->alert_id,data[i]->device_id,data[i]->plugin_id,data[i]->risk,data[i]->trojan_id,data[i]->alarm_time,data[i]->sip,data[i]->sport,data[i]->dip,data[i]->dport,data[i]->dmac,data[i]->protocol,,data[i]->descr,data[i]->dnshash,data[i]->scc,data[i]->scc_flag,data[i]->extra_text);
			...
			break;
		case 3://senstive_alert
			data = (struct senstive_alert**)data;
			for(i=0;i<size;i++)
			{
				memset(&(state->__ptr+i),0,sizeof(struct ns__MsgItem));
				memset(temp,0,sizeof(temp));
				sprintf( temp,"<event_id>%s</event_id><device_id>%s</device_id><event_class>%d</event_class><event_type>%s</event_type><event_time>%s</event_time><sip>%s</sip><sport>%d</sport><dmac>%s</dmac><dip>%s</dip><dport>%d</dport><sender>%s</sender><receiver>%s</receiver><title>%s</title><file_name>%s</file_name><descr>%s</descr><sen_msg>%s</sen_msg>",data[i]->event_id,data[i]->device_id,data[i]->event_class,data[i]->event_type,data[i]->event_time,data[i]->sip,data[i]->sport,data[i]->dmac,data[i]->dip,data[i]->dport,data[i]->sender,data[i]->receiver,data[i]->title,data[i]->file_name,data[i]->descr,data[i]->sen_msg);
			...
			break;
		case 4://session_blocked
			data = (struct session_blocked**)data;
			for(i=0;i<size;i++)
			{
				memset(&(state->__ptr+i),0,sizeof(struct ns__MsgItem));
				memset(temp,0,sizeof(temp));
