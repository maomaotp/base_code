...
#define JUDGE_TIME_ONESECOND 0
#define JUDGE_TIME_TENSECOND 1
#define JUDGE_TIME_ONEMIN    3
#define JUDGE_TIME_TEMMIN    4

#define JU
extern const char* prot_short_str[];
extern map<struct ipflag,struct ipscc>* ipmap;
extern char LOCALDEVID[11];
extern list<struct ip_rules> global_iplist;
extern uint32_t alarmstartip;
extern uint32_t alarmendip;
extern uint32_t alarmstartip1;
extern uint32_t alarmendip1;
unsigned int count = 1000;
bool ipflag::operator==( ipflag srcflag)
{
		if((startip <= srcflag.startip) && (endip >= srcflag.endip))
				return true;
		return false;
}

bool operator<(const ipflag desflag, const ipflag srcflag)
{
	//cout<<"插入!"<<endl;
		if(desflag.endip < srcflag.startip)
				return true;
		return false;
}

bool CInit_sql::ipmarkoff(in_addr_t *addr)
{
		u_int dst = *addr;
...
int CInit_sql::sql_netopt_info(cache_audit_t* netopt_data,
										  char *sql)
{
  if(judge_ip(netopt_data->dip, 0, netopt_data->dport))
		return 1;
  if(judge_ip(netopt_data->sip, 0, netopt_data->sport))
		return 1;

	uint32_t dstip = htonl(netopt_data->dip);
	uint32_t srcip = htonl(netopt_data->sip);
	...
	inet_ntop(AF_INET, &dstip, dststr, IP_STR_LEN);
	inet_ntop(AF_INET, &srcip, srcstr, IP_STR_LEN);

	char start_time[20];
	char end_time[20];
	memset(start_time, 0, 20);
	memset(end_time, 0, 20);
	TurnTime(netopt_data->start_time , start_time, 20);
	TurnTime(netopt_data->end_time , end_time, 20);
	char up_KB[100];
	char down_KB[100];
	char protocol[64];
	memset(up_KB, 0, sizeof(up_KB));
	memset(down_KB, 0, sizeof(down_KB));
	memset(protocol, 0, sizeof(protocol));
	if(6 == netopt_data->below_protocol)
	{
		strncpy(protocol, "TCP", 3);	
	}
	else if(17 == netopt_data->below_protocol)
	{
		strncpy(protocol, "UDP", 3);	
	}
	printf("protocol=%s\n", protocol);
//	sprintf(up_KB, "%.2f",netopt_data->up_KB);
//	sprintf(down_KB, "%.2f",netopt_data->down_KB);
//	printf("%s\n",up_KB);
//	printf("%s\n",down_KB);
//	rintf("yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy\n");
  sprintf(sql, "insert into tips_netlog(Dip, Dport, Sip, Sport,\
...
int CInit_sql::sql_alarm_info(cache_alarm_t * alarm_data,char *sql)
{
...
//  memcpy(judgetime, alarm_data->alarm_time, strlen(alarm_data->alarm_time)-JUDGE_TIME_ONEMIN);
...
#if 0
	if(judge_ip(dstiptemp, 2, 0))
		return 1;
	if(judge_ip(srciptemp, 2, 0))
		return 1;
#endif
	//uint32_t dstip = htonl(dstiptemp);
	//uint32_t srcip = htonl(srciptemp);
	...
	fprintf(stdout, "time1111111111111 = %s\n", judgetime);	
	/*if(type && (realsip > alarmstartip) && (realsip < alarmendip)){
		return 1;
	}*/
/*
	if(type && (((srciptemp > alarmstartip) && (srciptemp < alarmendip)) || ((dstiptemp > alarmstartip) && (dstiptemp < alarmendip)))){
		return 1;
	}
	if(type && (((srciptemp > alarmstartip1) && (srciptemp < alarmendip1)) || ((dstiptemp > alarmstartip1) && (dstiptemp < alarmendip1)))){
		return 1;
	}
*/
	fprintf(stdout, "time1111111111111 = %s\n", judgetime);	
//	if(!strlen(alarm_data->dmac))
	//	init_Dmac((judge ? srcip : dstip));
	fprintf(stdout, "time1111111111111 = %s\n", judgetime);	
	//init_scc((judge ? dstiptemp : srciptemp));
	fprintf(stdout, "time1111111111111 = %s\n", judgetime);	
	init_deviceID();
	//char srcstr[IP_STR_LEN];
	//char dststr[IP_STR_LEN];
