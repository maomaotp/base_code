...
int CMemcache::Init_Head(struct Mem_Head* memhead)
{
	...
	memhead->read_sign=memhead->write_sign=(struct Mem_Unit*)(memhead+1);
	memhead->write_sign->free_len=memhead->cache_size=size-sizeof(struct Mem_Head)-sizeof(struct Mem_Unit)*2;//*2保证最后一个存在
...
int CMemcache::Mem_Init(int cache_size,void* param)
{
	...
	size=block_size;
	write_head=head_one=(struct Mem_Head*)malloc(block_size);
	if(write_head==NULL)return 0;
	read_head=head_two=(struct Mem_Head*)malloc(block_size);
	if(read_head==NULL)return 0;
	Init_Head(head_one);
	Init_Head(head_two);
...
int CMemcache::Mem_Read(void* buff,int* len)
{
	...
	{
		if(read_finish==false)read_finish=true;
		return 0;
	}
	if(read_finish)return 0;//当前读取已经完成
	if(buff==NULL)return -1;
//	cout<<"当前数据长度::"<<read_head->read_sign->data_len<<"buff_len===="<<*len<<endl;
	if(read_head->read_sign->data_len>*len)return -2;//当前buff长度不够
...
int CMemcache::Mem_Change(int sec)
{
	if(sec!=0)
	{
		...
		//else cout<<"时间超时返回!"<<endl;
	}
	if(!read_finish)return -1;//当前读取没有完成
	write_flag=true;
	while(1)
	{
