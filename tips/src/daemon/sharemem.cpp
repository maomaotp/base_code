...
//		printf("typesize=%d\n", size);
		...
		if(shm->next_var_offset >= sizeof(shm->mem))
		{
...
int ShareMem::Set_status_shm(int id, const void * param, int len)
{
//	printf("var[%d].type=%d\n", id, shm->var[id].type);
//	printf("var[%d].num=%d\n", id, shm->var[id].num);
...
//	printf("size=%d\n", size);
	if(len != size)
	{
		dzlog_info("param length not right,so set status:%d failed\n", id);
		return -1;
	}
	char* ptr = shm->mem;
...
int ShareMem::Get_status_shm(int id, void * param, int len)
{
	...
	{
		dzlog_info("id:%d status had not write in shm\n", id);
		return -1;
	}
	int size = Sizeof(shm->var[id].type) * shm->var[id].num;
	if(len < size)
	{
		dzlog_info("your buffer not enough, so get status:%d failed\n", id);
		return -1;
	}
