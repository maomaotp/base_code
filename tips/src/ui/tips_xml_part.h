...
inline char *xml_Alarm_desc(char *value, char **xmlpart)
{
        char *temp = *xmlpart;
				if(value[strlen(value) - 1] == 'A'){
					value[strlen(value) - 1] = '\0';
				}
        *xmlpart = g_strdup_printf("%s<Alarm_desc><![CDATA[%s]]></Alarm_desc>",temp, value);
        g_free(temp);
        return *xmlpart;
}
...
inline char *xml_Protocol(char *value, char **xmlpart)
{
		...
		int i = 0;
		i = atoi(value);
		printf("temp,prot_short_str[i] is %s\n",prot_short_str[i]);
		if(i>118)
    			*xmlpart = g_strdup_printf("%s<Protocol>%s</Protocol>",temp,prot_short_str[0]);
		else
    			*xmlpart = g_strdup_printf("%s<Protocol>%s</Protocol>",temp,prot_short_str[i]);
#if 0 
				if(!strcmp(value, "6"))
...
inline int tips_netlog_params(params_t* params, netlog_stut_t *netlog_info)
{
	...
	netlog_info->Dport = (char *)params_value(params, "tips_Dport");
	...
	int i = atoi(netlog_info->Protocol);
	printf("111111111Protocol:%d\n",i);
//	exit(0);
	...
	netlog_info->Every_pagenum = (char *)params_value(params, "every_pagenum");
	return 0;
}
...
inline int tips_alarm_params(params_t* params, alarm_stut_t *alarm_info)
{
	...
	printf("qqqqqqqqqqA=%s\n",alarm_info->Alarm_time);
        alarm_info->Alarm_end_time = (char *)params_value(params, "tips_endtime");
        printf("qqqqqqqqqqEnd=%s\n",alarm_info->Alarm_end_time);
