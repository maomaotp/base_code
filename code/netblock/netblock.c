...
#define LOG_CONF "/usr/local/tipschecker/etc"
#define PID_FILE "/usr/local/tipschecker/etc/tipscheck/netblockid"
...
void WtPid()
{
	...
	dzlog_info("netblock start now pid:%d", pid);
	return;
}
...
{
#ifdef DEBUG
    printf("recv the signal start func: pcapbreak_loop\n");
#endif
    if(handle == NULL) {
...
int main(int argc, char* argv[])
{
	...
	//zlog
    if( dzlog_init(LOG_CONF, "netblock") ) {
