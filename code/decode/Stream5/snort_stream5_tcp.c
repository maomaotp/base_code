...
static void PrintStateMgr(StateMgr *s)
{
    printf("StateMgr:\n");
    printf("    state:          %s\n", state_names[s->state]);
    printf("    state_queue:    %s\n", state_names[s->state_queue]);
    printf("    expected_flags: 0x%X\n", s->expected_flags);
    printf("    transition_seq: 0x%X\n", s->transition_seq);
    printf("    stq_get_seq:    %d\n", s->stq_get_seq);
}

static void PrintStreamTracker(StreamTracker *s)
{
    printf(" + StreamTracker +\n");
    printf("    isn:                0x%X\n", s->isn);
    printf("    ts_last:            %u\n", s->ts_last);
    printf("    wscale:             %u\n", s->wscale);
    printf("    mss:                0x%08X\n", s->mss);
    ...
    printf("    r_nxt_ack:          %X\n", s->r_nxt_ack);
    printf("    r_win_base:         %X\n", s->r_win_base);
    printf("    seglist_base_seq:   %X\n", s->seglist_base_seq);
    printf("    seglist:            %p\n", (void*)s->seglist);
    printf("    seglist_tail:       %p\n", (void*)s->seglist_tail);
    printf("    seg_count:          %d\n", s->seg_count);
    printf("    seg_bytes_total:    %d\n", s->seg_bytes_total);
    printf("    seg_bytes_logical:  %d\n", s->seg_bytes_logical);
...
static void PrintTcpSession(TcpSession *ts)
{
    ...
    printf("Client Tracker:\n");
    PrintStreamTracker(&ts->client);
    printf("Server Tracker:\n");
...
static inline void EventInternal (uint32_t eventSid)
{
    ...
    printf("Stream5 raised internal event %d\n", eventSid);
...
static inline void EventNo3whs (Stream5TcpPolicy *s5TcpPolicy)
{
    ...
    s5stats.events++;
	printf("snort eventAdd stream5_no_3WHS\n");
...
static void SegmentFree (StreamSegment *seg)
{
    ...
    printf("Dumping segment at seq %X, size %d, caplen %d\n",
    ...
    snort_ip_p sip, snort_ip_p dip, uint16_t sp, uint16_t dp, uint32_t dir)
{
    ...
    do
    {
        ...
        if(footprint == 0)
        {
            printf("Negative footprint, bailing %d (0x%X - 0x%X)\n",
                        footprint, stop_seq, st->seglist_base_seq);
        ...
        printf("Attempting to flush %lu bytes\n", footprint);
        ...
        if(flushed_bytes == -1)
        {
            ...
            if(st->seglist)
            {
                printf("dumping entire seglist!\n");
                purge_all(st);
            }

            printf("setting st->seglist_base_seq to 0x%X\n", stop_seq);
...
static inline int flush_to_seq(
    ...
    snort_ip_p sip, snort_ip_p dip, uint16_t sp, uint16_t dp, uint32_t dir)
{
    printf("In flush_to_seq()\n");

    if ( !bytes )
    {
        printf("bailing, no data\n");
        return 0;
    }

    if ( !st->seglist_next )
    {
		printf("bailing, bad seglist ptr\n");
        return 0;
    }

    if (!DataToFlush(st) && !(st->flags & TF_FORCE_FLUSH))
    {
		printf("only 1 packet in seglist no need to flush\n");
        return 0;
    }
...
#define POST_SESSION_CLEANUP(delete_reason) \
    ...
    { \
        ...
        printf("S5: Pruned session from cache that was " \
...
midstream_pickup_allowed:
            ...
            {
                printf("Stream5: no data in packet (SYN only), no need to create lightweight session.\n");
        ...
        if (!lwssn)
        {
            printf("Stream5: Failed to retrieve session object.  Out of memory?\n");
            return -1;
        }
    }
    else
    {
            printf("Stream5: Retrieved existing session object.\n");
    }
        ...
        Stream5Expire(p, lwssn))
    {
        ...
        if (lwssn->ha_state.session_flags & SSNFLAG_RESET)
        {
            ...
            printf("Finished Stream5 TCP cleanly!\n");
        }
        else
        {
            PRE_SESSION_CLEANUP(lwssn);
            ...
            printf("Stream5 TCP session timedout!\n");
    ...
    if ( status & ACTION_DISABLE_INSPECTION )
    {
        ...
        printf("Stream5 Ignoring packet from %d. Session marked as ignore\n",
        ...
        Stream5TcpPolicy *s5TcpPolicy)
{
    ...
    if (tcpssn == NULL)
    {
        if (TCP_ISFLAGSET(p->tcph, TH_SYN) &&
            !TCP_ISFLAGSET(p->tcph, TH_ACK))
        {
            printf("Stream5 SYN PACKET, establishing lightweight"
                    "session direction.\n");
        ...
        else if (TCP_ISFLAGSET(p->tcph, (TH_SYN|TH_ACK)))
        {
            ...
            if ((lwssn->session_state == STREAM5_STATE_NONE) ||
                (lwssn->ha_state.session_flags & SSNFLAG_RESET))
            {
                printf("Stream5 SYN|ACK PACKET, establishing ,lightweight session direction.\n");
    ...
    if(p->packet_flags & PKT_FROM_SERVER)
    {
        printf("Stream5: Updating on packet from server\n");
        lwssn->ha_state.session_flags |= SSNFLAG_SEEN_SERVER;
        if (tcpssn)
        {
            talker = &tcpssn->server;
    ...
    {
        printf("Stream5: Updating on packet from client\n");
        ...
        (p->tcph->th_flags & TH_SYN))
    {
        ...
        printf("Got SYN pkt on reset ssn, re-SYN-ing\n");
    }
    ...
    if ((p->dsize) && TCP_ISFLAGSET(p->tcph, TH_SYN))
    {
        ...
        if (StreamGetPolicy(lwssn, s5TcpPolicy, FROM_CLIENT) !=
            STREAM_POLICY_MACOS)
        {
            printf("Got data on SYN packet, not processing it\n");
            //EventDataOnSyn(s5TcpPolicy);
        ...
        (TCP_STATE_LISTEN == talker->s_mgr.state) )
    {
        ...
        if(p->tcph->th_flags & TH_RST)
        {
            ...
            /* Reset not valid. */
            printf("bad sequence number, bailing\n");
            Discard();
