...
int GetLWSessionKey(Packet *p, SessionKey *key)
{
    ...
    return GetLWSessionKeyFromIpPort(GET_SRC_IP(p), p->sp,
        GET_DST_IP(p), p->dp,
        GET_IPH_PROTO(p), vlanId, mplsId, addressSpaceId, key);
}
...
void GetLWPacketDirection(Packet *p, Stream5LWSession *ssn)
{
    if(IS_IP4(p))
    {
        if ( p->ip4h->ip_src == ssn->client_ip )
        {
        ...
        else if ( p->ip4h->ip_dst == ssn->client_ip )
        {
            if  (GET_IPH_PROTO(p) == IPPROTO_TCP)
            {
...
#define s5_over_memcap() \
    ...
    if (memCheck && pruned)
    {
        printf("S5: Pruned %d sessions from cache for memcap. %d ssns remain.  memcap: %d/%d\n",
                               ...
                               const SessionKey *key, void *policy)
{
    ...
    if (!hnode) {
        printf("HashTable full, clean it\n");
    }
    if (hnode && hnode->data) {
        retSsn = hnode->data;
        ...
        retSsn->policy_id = runtimePolicyId;
